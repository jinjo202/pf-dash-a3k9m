...
"주간회의자료 만들어줘"는 **언급한 항목만 고치라는 뜻이 아니다.** 직전 회차를 템플릿으로 복사하므로
**말 안 한 섹션이 지난 자료 그대로 남는다** — 반복 지적된 실수.

→ **매크로(경제지표)·주요 보고서·센티먼트·수급·이벤트·Decision Point는 지시 없어도 매번 재작성.**
→ 회의 폴더의 **신규 브로커 PDF 먼저 확인**, `briefings/` 최신본으로 지수·매크로 교차검증.
→ 상세 체크리스트는 **AGENTS.md §2.5**.
