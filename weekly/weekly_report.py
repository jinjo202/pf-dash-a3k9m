...
def build(target, bench, macro, kr, pdata, daily, sentiment, template):
    ...
    # 시장 종가표 (우측 raw, 좌측 수식 자동) ─ 데이터 as_of 기준
    as_of_d = datetime.strptime(bench.get("as_of"), "%Y-%m-%d").date()
    lv = bench_levels(bench, as_of_d)
    lv["T"] = euro_stoxx_levels(as_of_d)  # 유럽
    for col, (ye, cur, mb, wkb) in lv.items():
        if ye is not None:
            _set(ws, "%s8" % col, round(ye, 2))
        if cur is not None:
            _set(ws, "%s9" % col, round(cur, 2))
        if mb is not None:
            _set(ws, "%s10" % col, round(mb, 2))
        if wkb is not None:
            _set(ws, "%s11" % col, round(wkb, 2))
    # 날짜 라벨
    ...
    # Factor 점검 — 내용 셀(D열) 갱신
    factors = build_factor_lines(macro, kr)
    # 우선 기존 D13:D36 영역 클리어(빈 문자열)
    for rr in range(13, 37):
        if ws["D%d" % rr].value is not None:
            ws["D%d" % rr] = None
    for name, (lines, start_row) in factors.items():
        for i, ln in enumerate(lines):
            _set(ws, "D%d" % (start_row + i), ln)

    # 주식 손익 (D=1~5월, E=6월, F=계)
    pnl = build_pnl(pdata)
    def put_pnl(row, key):
    ...
    put_pnl(41, "unrealized")  # FVPL 평가
    put_pnl(42, "realized")    # 매각/배당
    put_pnl(43, "pnl")         # 계

    # 국가·섹터 비중
    country, sec = build_weights(pdata)
    for row, ko in ((40, "한국"), (41, "미국"), (42, "유럽"), (43, "이머징"), (44, "기타")):
        if ko in country:
            _set(ws, "I%d" % row, round(country[ko], 4))
    for row, ws_label in ((40, "J40"), (41, "J41"), (42, "J42"), (43, "J43")):
        ko = ws[ws_label].value  # 기존 섹터 라벨(IT/산업재/금융/경기소비) 유지
        if ko in sec:
            _set(ws, "K%d" % row, round(sec[ko], 4))

    # □ 시장 센티먼트 (원본 하단에 덧붙임)
    add_sentiment_block(ws, sentiment, bench, macro, start_row=46)
