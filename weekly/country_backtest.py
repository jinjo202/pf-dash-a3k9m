...
def main():
    ...
    methodology = (
        ...
        "Value=후행 PER의 장기(10년 창) 중앙값 대비 괴리(쌀수록+; 자동 갱신 — "
        ...
        "PER만 싸진 경우) ②forward PER이 후행 대비 60% 미만이면 최대 70% 추가 차감 — "
        "경기민감주는 이익 정점에서 PER이 가장 낮게 보이므로(2026-08 한국 fwd 3.7 vs 후행 20.0, "
        "삼성 3.4·하이닉스 3.1이 EWY의 45%) 저PER을 저평가로 오독하지 않기 위한 장치. "
        "후행 PER은 bm-factors.js(Morningstar 집계)에 있는 한국·미국만 적용. "
        "한국은 수급 오버레이 별도 적용(외인 순매수·신용잔고 — "
