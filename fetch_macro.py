...
MONTHLY_FACTORS = {
    ...
    "2026-09": {
        "headline": "9월 FOMC 인하 여부가 최대 분수령 — 7월 반도체 급락·사상 최대 반등의 극단 변동성 이후 안정화 여부 시험대. 계절적 최약월 + 쿼드위칭 유의",
        "events": [
            {"date": "9/1", "title": "한국 8월 수출(관세청)", "dir": "watch", "sector": "반도체·한국",
             "impact": "7월 반도체 급락이 실물 수출에 번졌는지 확인하는 첫 지표. 단가(HBM·DDR5) 강세가 유지되면 급락은 수급·밸류 이슈였다는 방증 — 코스피 EPS 상향 흐름 지속 여부의 핵심."},
            {"date": "9/4", "title": "미국 8월 고용", "dir": "watch",
             "impact": "9월 FOMC 직전 마지막 고용지표. 7월 FOMC에서 3명이 인상을 주장하며 반대표를 던진 상황이라, 고용이 견조하면 인하 기대가 후퇴하고 매파 우위가 굳어진다."},
            {"date": "9/10 전후", "title": "미국 8월 CPI", "dir": "watch",
             "impact": "유가 $86대·관세 전가가 반영되는 구간. 코어가 흔들리면 9월 인하는 사실상 무산 — 고밸류 테크·장기채에 직격."},
            {"date": "9/15~16", "title": "FOMC", "dir": "neu",
             "impact": "올해 인하 사이클의 실질적 분수령. 인하 시 위험자산·한국 등 고베타 시장에 우호적이나, 동결+매파 점도표면 7월 급락의 재발 빌미. 잭슨홀(8월 말) 톤과의 일관성이 관건."},
            {"date": "9/18", "title": "쿼드러플 위칭(만기)", "dir": "watch",
             "impact": "선물·옵션 동시만기. 7월 디레버리징 이후 얇아진 포지션에서 만기 수급이 겹치면 변동성 증폭 — 방향보다 폭에 유의."},
            {"date": "9월 중", "title": "MSCI 분기 리뷰", "dir": "watch", "sector": "한국",
             "impact": "7월 급락으로 시총·유동성 기준이 변한 종목의 편출입 가능성. 패시브 리밸런싱 물량이 개별 종목 변동성으로 전이."},
            {"date": "9/25", "title": "추석 연휴(한국 휴장)", "dir": "watch", "sector": "한국",
             "impact": "휴장 중 발생한 해외 변수는 재개장일에 한꺼번에 반영. 연휴 전 포지션 축소 → 거래대금 감소·변동성 확대 패턴 반복 주의."},
            {"date": "9월 초", "title": "애플 신제품 발표", "dir": "neu", "sector": "IT·부품",
             "impact": "국내 부품·카메라·기판 밸류체인 모멘텀. 온디바이스 AI 사양 강화 여부가 관전 포인트."},
        ],
        "themes": [
            {"title": "🇰🇷 7월 극단 변동성 이후 — 정상화인가, 2차 하락의 전주곡인가", "dir": "watch",
             "outlook": "7월 코스피는 월초 8,303 → 급락 후 7/31 하루 +17.9%(사상 최대폭, +1,000pt) 반등해 6,595 마감 — 그럼에도 월간 −20.6%다. SK하이닉스·삼성전기 상한가, 삼성전자 +28%로 반등 자체가 정상 장세의 폭이 아니다. 급락·급등이 모두 펀더멘털이 아니라 레버리지 청산과 숏커버라는 수급 이벤트였다는 뜻. 9월 관전 포인트는 ①거래대금·변동성이 평시로 수렴하는지 ②반도체 단가·수출이 실물로 뒷받침하는지. 수급 이벤트로 끝나면 저가매수 국면, 실물이 꺾이면 2차 하락."},
            {"title": "🇰🇷 디레버리징 종료 이후 수급 — 신용잔고 37.4→32.5조", "dir": "pos",
             "outlook": "신용잔고가 5월 37.4조에서 7월 32.5조로 −4.9조 축소됐다. 강제 반대매매를 부르는 레버리지가 상당 부분 정리됐다는 의미 — 같은 악재에도 하방 증폭이 줄어든다. 예탁금 109.6조로 대기자금은 유지. 다만 신용이 다시 빠르게 늘면 취약성도 함께 복원되므로 재차 증가 속도를 감시."},
            {"title": "🇺🇸 9월 인하 여부 — 매파 반대표 3인의 무게", "dir": "watch",
             "outlook": "7월 FOMC에서 3명이 '인상'을 주장하며 동결에 반대했다. 단순 동결이 아니라 위원회 내부가 인플레 재가속을 경계하고 있다는 신호. 유가 $86대·관세 전가가 8월 CPI에 반영되면 9월 인하는 무산 가능성이 크다. 인하 기대에 기대어 밸류를 정당화해온 고멀티플 구간(CAPE 40대)에는 실망 리스크가 비대칭적으로 크다."},
            {"title": "AI capex — 4사 상향으로 사이클 연장, 그러나 집중도 위험", "dir": "pos",
             "outlook": "MS·아마존 등 하이퍼스케일러가 Q2 실적에서 capex 전망을 상향하며 AI 인프라 투자 지속을 확인했다(아마존 +15%, SOX +7.2%). 한국 메모리에는 직접 수요로 연결되는 호재. 다만 7월 급락이 보여줬듯 이 테마는 소수 종목에 시총이 쏠려 있어, 투자 사이클 자체가 아니라 한 종목의 가이던스 한 줄에 지수가 흔들린다. 방향은 긍정, 변동성은 각오할 것."},
            {"title": "🇰🇷 원화·외인 수급 — 환율 1,430대 안착 여부", "dir": "watch",
             "outlook": "원달러가 1,542 → 1,430대로 내려오며 외인 환손실 부담이 크게 줄었다. 환율 안정은 외인 순매수 전환의 선결 조건이라 긍정적. 다만 7/31처럼 증시가 사상 최대폭 급등했는데도 네고 부족으로 원화가 되레 약세를 보인 날이 있었다 — 환율과 증시의 연결이 평시만큼 견고하지 않은 국면이므로 단선적 해석은 금물."},
            {"title": "9월 계절성 — 역사적 최약월 + 얇아진 포지션", "dir": "neg",
             "outlook": "9월은 미국 증시의 역사적 최약월이다. 여기에 7월 디레버리징으로 포지션이 얇아진 상태, 쿼드위칭(9/18), 추석 휴장(9/25)이 겹친다. 방향성 베팅보다 이벤트 전후 유동성 공백에서의 변동성 관리가 우선."},
        ],
    },
    "2026-08": {
