...
3. 지수 통계 라인 — 반드시 "* " 로 시작(렌더러가 자동 처리). 국가 대표지수 3개를 한 줄로 묶는다. 정확한 예: "* 7.6(연초대비): 한국 △0.5%(+91.1), 일본 △0.0%(+38.5), 중국 △0.1%(+1.8)". 한국=코스피, 일본=닛케이225, 중국=상해종합. {M.D}=기준일 월.일. **앞 값은 chgPct 뒤에만 '%'를 붙이고(예 △0.5%), 괄호 안은 ytdPct 숫자만 넣는다(소수1자리, 부호만 — 예 (+91.1) 또는 (△10.2)). 괄호 안에 '%'·'ytd'·'연초' 같은 문자를 절대 붙이지 말 것.** 하락 △, 상승 +.
...
8. 환율·금리 문단(반드시 작성, 생략 금지): [환율·금리 실시간] 데이터를 사용한다.
   ① 원/달러 — 전일대비 등락(원)과 레벨을 쓰고(예 "△4.6원 내린 1,502.4원"; now·chg·chgPct 사용, chg 음수면 하락 △·양수면 상승 +), 방향의 배경을 한 마디 붙인다(달러 강·약, 위험선호/회피, 외인 수급 등 events·뉴스 근거).
   ② 국고채 10년 — 수익률 레벨과 전일대비 bp를 쓰고(예 "△2bp 내린 4.23%"; yield·bp 사용, bp 음수면 하락 △), 배경을 한 마디(미국 국채금리 연동·수급·정책 기대 등).
   ③ 단순 수치 나열 금지 — 두 항목 모두 '왜 움직였는지'를 짧게 덧붙인다. 배경이 데이터·뉴스로 불명확하면 억지 해석 말고 방향만 담백하게. 실시간 데이터가 없으면 그 항목만 생략.
...
- 지수 등락(chgPct)은 아래 [장중 잠정 지수(네이버 실시간)]의 값을 사용한다(daily-data 가 아니라 이것). 연초대비(ytdPct)는 daily-data regions 의 값을 병기한다.
- 제목 끝에 반드시 " (초안)" 을 붙인다. 총평 첫 문장에서 '장중 잠정치 기준'임을 밝힌다.
- 3번 지수 통계 라인 정확한 예: "* 7.6 장중잠정(연초대비): 한국 △0.5%(+91.1), 일본 △0.0%(+38.5), 중국 △0.1%(+1.8)". **괄호 안은 ytdPct 숫자만(소수1자리, 부호만) — '%'·'ytd'·'연초' 같은 문자를 절대 붙이지 말 것.** chgPct 뒤에만 '%'.
...
def build_request(region, daily, bench, override_as_of=None, draft=False):
    ...
    parts = [rules, RULES_READABILITY, RULES_REPORT, RULES_OUTLOOK]
