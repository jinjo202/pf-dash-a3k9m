...
window.FM_DATA = {
  ...
  // ── 5) 최근 정성 요인 (시장을 움직인 뉴스 + 본 계정 함의) ──
  // dir: pos | neg | watch  /  새 이벤트 발생 시 갱신(큐레이션)
  recent_factors: {
    as_of: "2026-06-27",
    note: "최근 시장을 움직인 정성 요인과 본 계정(AI·메모리·한국 집중) 함의. 큐레이션 — 새 이벤트 시 갱신.",
    items: [
      { date: "2026-06-24", tag: "메모리", dir: "pos",
        title: "마이크론 FQ3'26 어닝 서프라이즈 → 칩플레이션 확산",
        detail: "매출 4배↑ $41.46B·차분기 가이드 ~$50B, GAAP 매출총이익률 85%(전년 37.7%), DRAM ASP +32% QoQ. 마이크론 주가 +15%. CEO '메모리 공급난 2028까지 점진 개선'.",
        implication: "HBM·DRAM 가격 강세가 실적으로 확인 = 메모리 슈퍼사이클 지속 시그널 → 삼성전자·SK하이닉스 직접 수혜(핵심 한국 비중·HBM 테마 강화). 단 '칩플레이션'(메모리 원가의 전방 전가)은 양면 — 메모리 업스트림 비중확대 정당화 / 세트·다운스트림 마진·수요는 리스크.",
        affected: ["005930 KS","000660 KS","MU"] },
      { date: "2026-06", tag: "소비재/세트", dir: "watch",
        title: "애플, 메모리 원가 급등에 Mac·iPad 가격 인상",
        detail: "메모리 'hundred-year flood' 원가 상승을 제품가로 전가. 애플 주가 −6%, 10대 빅테크 중 마이크론만 YTD 상승.",
        implication: "칩플레이션이 실물로 번지는 실증 — 메모리 타이트가 세트가에 반영될 만큼 강하다는 확증(우리 메모리 비중에 우호). 동시에 세트 수요 탄력성·빅테크 마진 압박은 경계. 포트에 애플 직접 보유는 없으나 세트 수요→메모리 수요 연결고리 점검.",
        affected: ["005930 KS","000660 KS"] },
      { date: "2026-06-26", tag: "AI 센티먼트", dir: "neg",
        title: "OpenAI 상장 2027로 연기 보도",
        detail: "NYT: 2026 기대됐던 IPO를 2027로 연기 가능성(6/8 비밀 상장신청). SpaceX 상장 후 급등→급락에 리테일 수요 우려. 'AI 트레이드가 벽에 부딪혔다'.",
        implication: "AI 1차 대형 유동성 이벤트 지연 → AI 밸류·리스크 심리에 단기 부담(버블 후반부 징후 보강). 다만 빅테크·반도체 capex 펀더멘털과는 별개 — NVDA·AVGO·SMH 등 우리 AI 익스포저는 센티먼트 변동성 요인이자, 추후 상장 시 테마 재점화 촉매로 대기.",
        affected: ["NVDA","AVGO","SMH"] }
    ]
  },
