...
### 2.5 주간회의자료 = **전 섹션 갱신이 디폴트** ⚠️

"주간회의자료 만들어줘/업데이트해줘"는 **사용자가 언급한 항목만 고치라는 뜻이 아니다.**
직전 회차 파일을 템플릿으로 복사하기 때문에 **말하지 않은 섹션은 지난 자료가 그대로 남는다** — 이게 반복 지적된 실수다.

**매 회차 반드시 재작성할 섹션 (지시 없어도 기본)**

| 섹션 | xlsx 위치 | 갱신 기준 |
|---|---|---|
| 기업이익 | D13~ | 직전 회차 이후 발표된 실적 전부 |
| **경제지표(매크로)** | D18~ | FOMC·CPI·고용·유가·환율·금리를 **당일 수치로** — VIX·10Y·WTI는 특히 잘 낡는다 |
| 수급 | D22~ | 신용잔고·공매도잔고(KRX 실측)·외국인·레버리지 ETF |
| 주요 이벤트 | D25~ | 지난 이벤트는 "완료" 처리하고 다음 주 캘린더로 교체 |
| **센티먼트** | D32~ | ERB·VIX·투심 지표. 지수 급변 시 반드시 재작성 |
| **주요 보고서** | D34~ | **직전 1주 이내 발간분만.** 회의 폴더의 신규 PDF를 먼저 확인 |
| Decision Point | D38~ | 종합판단·실행·자기검증 전면 재작성 |
| 시장변동표 | B9/D9~N9, R9~AB9 | 기준일 종가 |

**작업 순서**
1. 회의 폴더(`OneDrive/SS주간 발표자료/주간 회의자료/<YYYYMMDD>`)에 **새로 들어온 브로커 PDF부터 확인** → 있으면 주요 보고서는 그걸로 교체
2. `briefings/` 최신 파일로 매크로·지수 교차검증 (yfinance는 한국 지수 당일치를 자주 놓친다)
3. 전 섹션 재작성 후 **날짜·수치가 직전 회차에서 안 넘어왔는지 역검색**:
   ```bash
   # 예: 지난달 수치가 남아있는지
   grep -o "5\.8배\|7/17\|VIX 18\.8" <xlsx에서 추출한 텍스트>
   ```
4. `python weekly/build_weekly_data.py <meeting> <target> <xlsx>` 로 weekly-data.js 동기화

**금지**: 사용자가 특정 섹션만 언급했다고 나머지를 그대로 두는 것. 언급은 *강조*이지 *범위 한정*이 아니다.
...
7. **주간회의자료 stale 섹션**: 직전 회차 템플릿을 쓰므로 **말 안 한 섹션(매크로·주요보고서·센티먼트)이 그대로 남는다** → §2.5대로 전 섹션 재작성이 디폴트.
