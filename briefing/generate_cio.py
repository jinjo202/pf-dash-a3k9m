...
FM_ETF_JS = os.path.join(REPO, "fm-etf.js")
...
def etf_universe(fm):
    ...
    # fm-etf.js의 수익률(1M/3M/YTD)을 유니버스에 병합 — 부진(언더퍼폼) 판별 근거.
    etf = load_js_var_node(FM_ETF_JS, "FM_ETF") or {}
    edata = etf.get("data") or {}
    for row in out:
        e = edata.get(row["ticker"]) or {}
        for k in ("r_1m", "r_3m", "r_ytd"):
            if e.get(k) is not None:
                row[k] = e[k]
        if e.get("benchmark"):
            row["benchmark"] = e["benchmark"]
    return out
...
5) **매매는 ETF 단위로 실행된다.** 이 계정은 개별 섹터 미세조정이 어렵고 실제 매매는 ETF로 이뤄진다.
   ...
   ⚠ **너는 현재 이 계정의 실제 보유 종목·비중을 알 수 없다.** 따라서 trades에서 '유지/축소' 같이 보유를
   전제하는 단정을 하지 마라. 각 trade는 그 ETF/노출에 대한 방향성(lean)만 판정한다:
   lean="bullish"=늘릴 방향(강세), lean="bearish"=줄일 방향(약세). 실제 보유 여부와 현재 비중을 대조해
   '유지·확대 / 축소 / 신규 편입 / 회피'로 최종 라벨링하는 것은 프런트엔드(복호화된 포트폴리오)가 한다.
   reason에는 이 방향을 그 ETF로 실행하는 근거를 쓴다. 유니버스에 적절한 ETF가 없으면 trades 대신
   sector/country 서술에 방향만 남긴다.
6) **부진(언더퍼폼) ETF 점검 — watchlist.** [etf_universe]의 각 ETF에는 수익률(r_1m/r_3m/r_ytd %)이 있다.
   최근 부진한(3M 수익률이 낮거나 마이너스, 또는 동종 대비 뚜렷이 뒤처지는) ETF 2~4개를 골라, 왜 부진한지와
   홀드/매도 판단을 구체적으로 제시한다(예: 방산 ETF가 3M 마이너스면 왜인지·기다릴지·팔지). 각 항목:
   why_lagging(부진 원인 — 데이터·뉴스 근거), hold_thesis(그럼에도 보유/기다릴 근거),
   wait_until(언제까지·어떤 조건까지 기다릴지 — 구체 시점이나 지표 레벨), sell_triggers(이 신호가 나오면 매도 — 1~3개).
   판단(verdict)은 hold(기다림)|trim(일부 축소)|sell(청산). 막연히 '지켜본다' 금지 — 시한과 트리거를 명시.
...
[출력 JSON 스키마 — 키 이름 정확히]
{
 ...
 "positioning": {
   ...
   "trades": [{"lean":"bullish|bearish","ticker":"ETF티커([etf_universe] 내)","name":"...","reason":"이 방향(강세=늘림/약세=줄임)을 이 ETF로 실행하는 근거. '유지'라 단정 말 것 — 보유 여부는 프런트가 대조"}]
 },
 "watchlist": [
   {"ticker":"ETF티커([etf_universe] 내)","name":"...","r_3m":숫자또는null,
    "verdict":"hold|trim|sell","why_lagging":"부진 원인 1~2문장(데이터·뉴스 근거)",
    "hold_thesis":"기다릴/보유 근거 1~2문장","wait_until":"언제까지·어떤 조건까지(구체 시점·지표 레벨)",
    "sell_triggers":["매도 트리거1","매도 트리거2"]}
 ],
 "conclusion": "종합 결론 문단 — 오늘 무엇을 하고 무엇을 하지 않는지 + 판단을 바꿀 트리거 2~3개"
}
chain 4~8개, factors 2~5개(오늘 뉴스로 확인된 신규/갱신만), trades 0~5개(없으면 빈 배열), watchlist 2~4개(부진 ETF 없으면 빈 배열).
...
- watchlist의 홀드/매도 판단: 부진 ETF를 '기다린다'는 게 매몰비용 합리화 아닌가, wait_until 시한이 너무 느슨하지 않은가, sell_trigger가 실제 실행 가능한 구체 신호인가
...
def main():
    ...
    # 모델이 습관적으로 옛 action(유지/축소/…)을 낼 경우 lean으로 정규화(프런트가 보유 대조).
    pos = result.get("positioning") or {}
    for t in (pos.get("trades") or []):
        if not t.get("lean"):
            a = t.get("action") or ""
            t["lean"] = "bearish" if any(k in a for k in ("축소", "매도", "헤지", "회피", "약세")) else "bullish"
        t.pop("action", None)
